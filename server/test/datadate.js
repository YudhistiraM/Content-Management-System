...
describe('datadate', function(){
  ...
  it("Mecari data nilai letter dan frequency serta mengembalikannya dengan metode POST", function(done){
    chai.request(server)
  ...
  it("Seharusnya sistem mengembalikan nilai id, letter dan frequency dengan metode GET", function(done){
    chai.request(server)
    .get('/api/datadate')
    .end(function(err, res){
      res.should.have.status(200);
  ...
  it("Seharusnya sistem mengembalikan berhasil mengubah data dengan metode PUT", function(done){
    ...
    .end(function(err, res){
      ...
      .send({
        ...
        response.body.data.letter.should.equal("2018-09-21");
