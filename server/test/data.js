...
describe('data', function(){
  ...
  it("Seharusnya menambahkan satu data dengan metod POST", function(done){
    chai.request(server)
    .post('/api/data/')
    .send({
      letter: 'Camp',
      ...
      res.body.data.letter.should.equal("Camp");
      res.body.data.frequency.should.equal(100.5);
      done()
    })
  })

  it("Mencari data nilai letter dan frequency serta mengembalikannya dengan metode POST", function(done){
    chai.request(server)
  ...
  it("Seharusnya sistem mengenbalikan nilai id, letter dan frequency dengan metode GET", function(done){
    chai.request(server)
    .get('/api/data')
    .end(function(err, res){
      res.should.have.status(200);
